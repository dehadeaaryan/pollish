...
import { CancelIcon, CheckIcon, ClipboardIcon, EditIcon, HeartFilledIcon, HeartSlashIcon, MailIcon, MessageCircleIcon, SaveIcon, SendIcon, TrashIcon } from "@/components/icons";
...
function PollishCard({ pollish_id, props, setCards, cards }: PollishCardComponentProps) {
    ...
    return (
        <motion.div
            ...
            style={{
                gridRow: 1, gridColumn: 1, x, rotate, transition: "0.125s transform", boxShadow: isFront ? "0 20px 25px -5px rgb(0 0 0 / 0.5), 0 8px 10px -6px rgb(0 0 0 / 0.5)" : undefined,
            }}
            animate={{
        ...
        >
            ...
            >
                ...
                <Image
                    ...
                    style={{ cursor: isEditing ? "pointer" : "cursor-grab active:cursor-grabbing", opacity: isEditing ? 0.6 : 1 }}
                />
                <input
                    type="file"
                    accept="image/*"
                ...
                <CardFooter className="absolute bg-black/40 bottom-0 z-10 border-t-1 border-default-600 dark:border-default-100 w-full flex justify-between items-center gap-2 p-3">
                    {isEditing ? (
                        <>
                            <Button size="sm" radius="lg" color="default" onPress={handleCancel}>
                                <CancelIcon size={24} />
                            </Button>
                            <Button size="sm" radius="lg" color="success" onPress={handleSave}>
                                <SaveIcon size={24} />
                            </Button>
                        </>
                    ) : (
                        <>
                            <Popover placement="top" showArrow isOpen={isPopoverOpen} onOpenChange={(open) => setIsPopoverOpen(open)} backdrop="blur">
                                <PopoverTrigger>
                                    <Button size="sm" radius="lg" color="danger"><TrashIcon size={24} /></Button>
                                </PopoverTrigger>
                                <PopoverContent className="w-[220px] p-4 text-center">
                                    <p className="text-sm mb-3">Are you sure you want to delete this card?</p>
                            ...
                            <Button size="sm" radius="lg" color="warning" onPress={() => setIsEditing(true)}>
                                <EditIcon size={24} />
                            </Button>
                        </>
                    )}
...
export default function EditPollishPage() {
    ...
    const handleDeleteAction = async () => {
        if (!confirm("Are you sure you want to delete this Pollish? This action cannot be undone.")) return;
    ...
    const handleBack = () => {
        if (cards.length === 0) return;
        const backCards = [cards[cards.length - 1], ...cards.slice(0, cards.length - 1)];
        setCards(backCards);
    };
    ...
    useEffect(() => {
        ...
        const fetchAssets = async () => {
            try {
                ...
                const backgroundResponse = await axios.get(`${api_gateway_url}/background`, {
                    params: { pollish_id: id },
                    headers: {
                        "x-api-key": api_gateway_key!,
                    },
        ...
        const fetchPollish = async () => {
            try {
                ...
                const assetsResponse = await axios.get(`${api_gateway_url}/generate-download-url`, {
                    params: { id },
                    headers: { "x-api-key": api_gateway_key! },
                });
    ...
    return (
        <div
            ...
            <h1 className={title({ color: "yellow", size: "lg" }) + " capitalize z-30 pb-2"}>{pollishName}</h1>
            <div className="grid place-items-center h-full z-10">
                {cards.length > 0 ? (
                    ...
                    >
                        <Card onPress={() => setIsPopoverOpen(true)}
                            isBlurred
                            ...
                            <CardBody className="grid place-items-center h-full">
                                <p className="text-center text-large font-medium">Your cards go here</p>
                            </CardBody>
                        </Card>
                    </motion.div>
                )}
            ...
            <div className="w-full flex justify-center items-center gap-2">
                ...
                <Dropdown className="z-30">
                    <DropdownTrigger>
                        <Button variant="solid" color="default" size="md" radius="lg">Options</Button>
                    </DropdownTrigger>
                    <DropdownMenu
                ...
                <Button className="z-30" color="default" size="sm" radius="lg" onPress={handleBack}>
                    {"<"}
                </Button>
                <Button className="z-30" color="default" size="sm" radius="lg" onPress={handleNext}>
                    {">"}
                </Button>
                <Popover showArrow offset={10} placement="bottom" isOpen={isPopoverOpen} onOpenChange={(open) => setIsPopoverOpen(open)}>
                    <PopoverTrigger>
                        <Button onPress={() => setIsPopoverOpen(true)} className="z-30" color="primary" size="md" radius="lg">
                            Add
                        </Button>
                    </PopoverTrigger>
                                ...
                                <div className="px-1 py-2 w-full">
                                    ...
                                    <div className="mt-2 flex flex-col gap-2 w-full">
                                        <Input
                                            ...
                                            variant="bordered"
                                        />
                                        <Button color="success" size="sm" onPress={handleCreateCard}>
                                            Create
                                        </Button>
                                    </div>
                                </div>
                ...
                <ModalContent>
                    {(onClose) => (
                        <>
                            ...
                            <ModalFooter>
                                <Button size="md" radius="lg" variant="light" onPress={onClose}>
                                    Cancel
                                </Button>
                                <Button size="md" radius="lg" color="primary" onPress={handleCopy}>
                                    Copy Link
                                </Button>
                            </ModalFooter>
                        </>
